...
angular.module('hm.fileupload',[
    ...
    return function(file,dpi1,dpi2){
        return file.fileurl==null?(FILESERVICE+file.systemName+'/'+file.dir+'/'+(dpi1==null?"":(dpi1+'_'))+(dpi2==null?"":(dpi2+'_'))+file.saveName+'/'+file.fileName):file.fileurl;
    }
}]);
...
function fileUploadService ($http,$q,Upload,FILESERVICE) {
    ...
    function batchDownload(fileList){
        var deferred = $q.defer();
        $http.post(
            fileHost+'batchFile/download',
            fileList,
            {transformResponse:function(data){
              return fileHost+'batchFile/'+data;
            }}
...
angular.module('hm.fileupload').directive( "hmUploadFile", ['$compile','$http','$window','fileUploadService','FILESERVICE','FILEDIRNAME','FILESYSTEMNAME',
    ...
    return {
        ...
        link:function( scope, element, attrs, ngModel ){
            ...
            var batchDownload=attrs['batchdownload'];

            var tmp='<div class="file-upload-mask hide"></div><div class="file-upload-bar">' +
                '       <div ng-repeat="file in ngModel" class="file-upload-info">' +
                '           <div ng-if="file.saveName==null" class="file-upload-ing">' +
                '               <div class="file-progress">' +
                '                   <div style="margin-top: 30px">' +
                '                       <span>{{file.progress}}</span>' +
                '                   </div>' +
                '                   <div class="progress progress-striped active ">' +
                '                       <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: {{file.progress}}"></div>' +
                '                   </div>' +
                '               </div>' +
                '           </div>' +
                '           <div ng-if="file.saveName!=null">' +
                '               <div ng-if="(file.fileName | isImage)" class="file-type-img" style="background: url(\'{{file | backgroundUrl }}\') #000 center no-repeat;">' +
                '                   <span class="ng-binding">&nbsp;</span>' +
                '                   <div class="file-upload-menu file">' +
                '                       <div class="arrow"></div>' +
                    '                   <div class="menu-btn edit" ng-click="{editFuncName}(file)"></div>' +
                    '                   <div class="menu-btn download" ng-click="{downloadFuncName}(file)"></div>' +
                    '                   <div class="menu-btn del" ng-click="{delFuncName}(file)"></div>' +
                '                   </div>' +
                '               </div>' +
                '               <div ng-if="!(file.fileName | isImage)" class="file-type-file">' +
                '                   <span ng-if="(file.fileName | getFileType)!=\' \'" class="ng-binding">{{file.fileName | getFileType}}</span>' +
                '                   <span ng-if="(file.fileName | getFileType)==\' \'" class="ng-binding">&nbsp;</span>' +
                '                   <div class="file-upload-menu file">' +
                '                       <div class="arrow"></div>' +
                '                       <div class="menu-btn edit" ng-click="{editFuncName}(file)"></div>' +
                '                       <div class="menu-btn download" ng-click="{downloadFuncName}(file)"></div>' +
                '                       <div class="menu-btn del" ng-click="{delFuncName}(file)"></div>' +
                '                   </div>' +
                '                   <div class="file-name">{{file.fileName | uriDecode}}</div>' +
                '               </div>' +
                '           </div>' +
                '       </div>' +
                '   <div class="file-upload-add"></div>' +
                '</div>'+(batchDownload=='true'?'<div ng-show="ngModel.length>1" class="file-upload-batch-download">批量下载<div>':'');
            ...
            element.find('.file-upload-batch-download').on('click',function(){
                element.find('.file-upload-mask').removeClass('hide');
                fileUploadService.batchDownload(ngModel.$modelValue).then(function(data){
                    var elemIF = document.createElement("iframe");
                    elemIF.src = data;
                    elemIF.style.display = "none";
                    document.body.appendChild(elemIF);
                    $('.file-upload-mask').addClass('hide');
                },function(){
                    toastr.error('打包失败','批量下载-失败');
                    $('.file-upload-mask').addClass('hide');
                });
            });
                    ...
                    for(var i=0;i<ngModel.$modelValue.length;i++){
                        if(file.saveName==ngModel.$modelValue[i].saveName){
                            if(file.fileurl!=null){
                                ...
                                url = fileHost+file.systemName+'/'+file.dir+'/'+file.saveName+'/'+encodeURIComponent(file.fileName);
                            }
                            break;
                        }
                    }
    ...
    function( $compile,$http,$window,fileUploadService,FILESERVICE,FILEDIRNAME ) {
        return {
            ...
            link:function( scope, element, attrs, ngModel ){
                ...
                var tmp='<div class="file-upload-bar">' +
                    '       <div ng-repeat="file in ngModel" class="file-upload-info detail">' +
                    '           <div ng-if="file.saveName!=null">' +
                    '               <div ng-if="(file.fileName | isImage)" class="file-type-img" style="background: url(\'{{file | backgroundUrl }}\') #000 center no-repeat;">' +
                    '                   <span class="ng-binding">&nbsp;</span>' +
                    '                   <div class="file-upload-menu file">' +
...
angular.module('hm.fileupload').directive( "hmUploadImage", ['$compile','$http','$window','$filter','fileUploadService','FILESERVICE','FILEDIRNAME','FILESYSTEMNAME',
    function( $compile,$http,$window,$filter,fileUploadService,FILESERVICE,FILEDIRNAME,FILESYSTEMNAME ) {
    return {
        scope:{
            ngModel: '='
        },
        ...
        link:function( scope, element, attrs, ngModel ){
            ...
            function imgURL(file,dpi){
                return function(file,dpi){
                    return file.fileurl==null?(FILESERVICE+file.systemName+'/'+file.dir+'/'+(dpi==null?"":dpi)+file.saveName+'/'+file.fileName):file.fileurl;
                }
            }
            ...
            var tmp='<div class="file-upload-mask hide"></div><div class="file-upload-bar">' +
                        ...
                        '   </div>' +
                        '   <div ng-if="file.saveName!=null">' +
                        '       <div class="file-type-img" style="background: url(\'{{file | backgroundUrl{dpi1}{dpi2} }}\') #000 center no-repeat;">' +
                        // '       <div class="file-type-img" style="background: url({file.fileUrl}) #000 center no-repeat;">' +
                        '           <span class="ng-binding">&nbsp;</span>' +
                        '           <div class="file-upload-menu">' +
                        '               <div class="arrow"></div>' +
                        '               <div class="menu-btn cover" ng-class="{\'active\':file.isCover==1}" ng-class="{\'active\':fileObj.size==1}" ng-click="{coverFuncName}(file)"></div>' +
                        '               <div class="menu-btn edit" ng-click="{editFuncName}(file)"></div>' +
                        '               <div class="menu-btn download" ng-click="{downloadFuncName}(file)"></div>' +
                        '               <div class="menu-btn del" ng-click="{delFuncName}(file)"></div>' +
                        '           </div>' +
                        '           <div ng-if="file.isCover==1 || fileObj.size==1" class="cover-tab"></div>' +
                        '       </div>' +
                        '   </div>' +
                        '</div>' +
                        '<div class="file-upload-add image"></div>' +
                        '</div>'+(batchDownload=='true'?'<div ng-show="ngModel.length>1" class="file-upload-batch-download">批量下载<div>':'');
             tmp=tmp.replace('{fileHost}',fileHost)
                    .replace(/\{coverFuncName}/g,coverFuncName)
                    .replace(/\{editFuncName}/g,editFuncName)
                    .replace(/\{downloadFuncName}/g,downloadFuncName)
                    .replace(/\{delFuncName}/g,delFuncName)
                    .replace(/\{dpi1}/g,dpiStr==''?'':':'+dpiStr.split('_')[0])
                    .replace(/\{dpi2}/g,dpiStr==''?'':':'+dpiStr.split('_')[1]);
            ...
            inputObj.on('change',function(e){
                ...
                if(_changeIndex==null && _changeFile==null && ngModel.$modelValue.length>=fileListSize){
                    toastr.error('最多上传'+fileListSize+'张图片!', '添加图片-失败');
                    return;
                }
                var fileList=e.target.files;
                for(var i=0;i<fileList.length;i++){
                    ...
                    if(typeList!=null && typeList!=""){
                        if(typeList.split(',').indexOf(FileUploadUtil().getType(fileObj.name))==-1){
                            toastr.error('请上传'+typeList.replace(/,/g,' ')+'类型的附件!', '添加图片-失败');
                            return;
                        }
                    }
                    if(fileSize!=null && fileSize!=""){
                        if(fileObj.size>fileSize){
                            toastr.error('上传的图片大小不能超过'+fileSizeStr+'!', '添加图片-失败');
                            return;
                        }
                    }
                    doUpload(fileObj,_changeIndex,_changeFile);
                    ...
                    for(var i=0;i<ngModel.$modelValue.length;i++){
                        if(file.saveName==ngModel.$modelValue[i].saveName){
                            if(file.fileurl!=null){
                                fileUploadService.deleteFile(file).then(function(){
                                    ngModel.$modelValue.splice(i,1);
                                    if(file.isCover==1 && ngModel.$modelValue.length>0){
                                        ngModel.$modelValue[0].isCover=1;
                                    }
                                    console.log('临时文件 删除成功');
                                },function(){
                                    console.log('临时文件 删除失败');
            ...
            if(typeof scope[downloadFuncName] == 'undefined'){
                scope[downloadFuncName]=function(file){
                    ...
                    for(var i=0;i<ngModel.$modelValue.length;i++){
                        if(file.saveName==ngModel.$modelValue[i].saveName){
                            if(file.fileurl!=null){
                                url = fileHost+'temp/'+file.systemName+'/'+file.dir+'/'+file.saveName+'/'+encodeURIComponent(file.fileName);
                            }else{
                                url = fileHost+file.systemName+'/'+file.dir+'/'+file.saveName+'/'+encodeURIComponent(file.fileName);
                            }
                            break;
                        ...
                        if(response.data!=null){
                            ...
                            if(fileList!=null){
                                for(var i=0;i<fileList.length;i++){
                                    ...
                                    /*判断是否第一张，并自动设置为封面图*/
                                    if(_changeFile!=null){
                                        fileInfo.isCover=(ngModel.$modelValue.length==1 || _changeFile.isCover==1)?1:0;
                                    }else{
                                        fileInfo.isCover= (ngModel.$modelValue.length==1)?1:0;
                                    }
                                }
                            }
                            if(_changeFile!=null){
        ...
        return {
            ...
            link:function( scope, element, attrs, ngModel ){
                ...
                var tmp='<div class="file-upload-bar">' +
                    ...
                    '                       <div class="menu-btn download" ng-click="{downloadFuncName}(file)"></div>' +
                    '                   </div>' +
                    '                   <div ng-if="file.isCover==1" class="cover-tab"></div>' +
                    '               </div>' +
                    '           </div>' +
                    '   </div>' +
                    '</div>';
        ...
        if(str!=null){
            ...
            for(var i=0;i<list.length;i++){
                switch(list[i]){
                    ...
                    case "audio":typeList=typeList.concat(AUDIO_TYPE);break;
